...
import {
  Star, ExternalLink, Heart, Bookmark, BookMarked, Frown, Settings, Pin, PinOff, Loader2, DownloadCloud, Dices, UserCircle2, Edit, Trash2, CalendarDays, Weight, PenTool, Clock, Medal, Sparkles, Trophy, Users, ListPlus, ListChecks, Wand2,
  MessageSquare
} from 'lucide-react';
...
interface GameDetailHeaderProps {
  ...
  userReview: Review | undefined; // Keep for button text and score display
  userOverallScore: number | null; // Score for the current user's review
      ...
      <div className="flex flex-col md:flex-row">
        ...
        <div className="flex-1 p-6 space-y-4 md:order-1">
          ...
          <div className="flex justify-between items-start mb-2">
            <div className="flex-1 min-w-0 mr-2"> {/* Container for title and BGG link */}
          ...
          {/* Button Bar */}
           <div className="border-t border-b border-border">
            <div className="flex justify-evenly items-center gap-1 sm:gap-2 py-4">
              <Button
                asChild
                variant="ghost"
                size="sm"
                title={currentUser && userReview ? "Modifica il Tuo Voto" : "Valuta questo Gioco"}
                className={cn(
                  'h-9 px-2 text-primary hover:text-primary/80 hover:bg-primary/10',
                  currentUser && userReview && 'border border-primary/50'
                )}
                // Always enabled
              >
                <Link href={`/games/${game.id}/rate`}>
                  <Star className="h-5 w-5" />
                  {currentUser && userReview && userOverallScore !== null ? (
